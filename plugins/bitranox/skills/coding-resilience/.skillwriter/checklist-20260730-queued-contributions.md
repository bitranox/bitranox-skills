# skill-writer checklist - coding-resilience: ship queued cross-project contributions (2026-07-30)
...
- [x] Evidence, verbatim where quoted: given a `set -euo pipefail` loop over 38 INDEPENDENT sync tasks with the symptom "sometimes only
      some apps update, no errors", haiku blamed subshells, pipelines and backgrounding - "Check: are
      tasks backgrounded (trailing &)? Piped?" - and never identified that `set -e` aborts the loop at
      the first failure so every later task silently never runs. Wrong root cause on the real symptom.
