...
Not for skills you write from scratch (use `bitranox:skill-writer`) and not for a skill that
...
**REQUIRED: `bitranox:skill-writer`.** Bring the adopted skill up to standard with it -
...
`bitranox:using-bitranox-skills`, bump `plugins/bitranox/.claude-plugin/plugin.json` one MINOR
...
**REQUIRED: `bitranox:self-improve`, its "Propagating skill (or hook) improvements upstream"
