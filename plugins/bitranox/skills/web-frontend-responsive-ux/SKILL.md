...
**The sweep is STATELESS: each profile is a fresh context + fresh page load.** It never
interacts and then rotates, so it cannot catch a *stateful* rotate bug - e.g. a JS-driven
zoom/pan viewer that keeps its old zoom when the phone is rotated (see the re-fit pattern
below). For any page with a live canvas/media viewer, add a manual check: load in one
orientation, drive the viewer, THEN rotate (`page.set_viewport_size(...)` + dispatch
`orientationchange`+`resize`) and assert the content re-fits.
...
- **Re-fit a live JS viewer on rotate/resize - it does NOT re-fit itself like CSS.** A static image with
  `object-fit:contain` re-fits automatically when the container reshapes (phone landscape<->portrait, window
  resize), but a JS-driven canvas viewer (OpenSeadragon, a map, a custom zoom/pan surface) keeps its OLD zoom
  and is left the wrong size for the new shape. Re-fit explicitly on `resize`/`orientationchange`, but ONLY if
  the user was at the fit (`getZoom <= getHomeZoom + eps`, captured in the window handler that fires BEFORE the
  viewer polls the new size) - preserve a zoomed-in view. The non-obvious mechanics (learned from OpenSeadragon):
  the viewer applies the new container size AFTER its own `resize` event and may ANIMATE to a wrong zoom, so a
  single snap in the resize handler gets clobbered. Instead POLL frames (`requestAnimationFrame` until
  `viewer.isAnimating()` is false, with a small min-delay so the animation has started and a hard cap), THEN
  snap to the fit (`goHome`). Polling also covers `prefers-reduced-motion` (no animation, hence no
  animation-finish event to hook). Avoid a shared boolean guard (e.g. a `recentering` flag) that an unrelated
  animation's finish can steal; prefer state-based guards (already-fitted AND already-centered) that make a
  re-entrant re-fit a natural no-op. The stateless matrix sweep will NOT catch this - test it with a stateful
  rotate (see "Audit across the matrix").
...
| A live JS viewer looks wrong-sized after rotating | It does not re-fit on container reshape like CSS `object-fit` does - it keeps the old zoom. Re-fit on `resize`/`orientationchange` (poll until the viewer settles, then `goHome`), only if the user was fitted. The stateless sweep misses it - test a stateful rotate |
