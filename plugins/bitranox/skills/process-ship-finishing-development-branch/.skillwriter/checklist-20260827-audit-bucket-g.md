# skill-writer checklist - process-ship-finishing-development-branch (2026-08-27, audit bucket G)

The self-announcement names a pre-rename slug.
...
- [x] Behavioural RED deliberately NOT used: these skills are INSTALLED on this machine, so a probe
      ...
      inherited context.
- [x] Same defect and same evidence as `process-plan-writing-plans`: the bare short form
      `finishing-development-branch` appears nowhere in the repo outside this one announcement
      line, while every cross-reference uses the prefixed name.
