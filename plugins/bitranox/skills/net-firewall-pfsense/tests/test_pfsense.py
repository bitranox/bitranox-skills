...
def test_doctor_warns_when_the_resolver_has_been_taken_over():
    findings = P.doctor_findings(reservations=[], overrides=[], arp=[],
                                 resolv_conf="nameserver 100.100.100.100\n")
    assert [f.check for f in findings] == ["resolv_conf_hijacked"]


def test_a_firewall_resolving_through_itself_is_not_flagged():
    findings = P.doctor_findings(reservations=[], overrides=[], arp=[], resolv_conf="nameserver 127.0.0.1\n")
    assert findings == []
