# skill-writer checklist - compuse-ssh: ship queued cross-project contributions (2026-07-30)
...
- [x] VERDICT: FAILED on haiku (weak-model re-run)
- [x] Evidence, verbatim where quoted: haiku wrote `ssh -o ... host true` and asserted "Works on both: Linux shells and Windows
      PowerShell/cmd all have the `true` built-in" - factually wrong, cmd.exe has no `true`, which is
      the exact trap. On the file read it caught the host-key PROMPT but concluded "The byte-compare
      itself is fine", missing the merged-stderr contamination. sonnet passed both, so the capable
      model was masking a real gap - the documented reason to re-run weak.
- [x] The Alpine `adduser -D` row ships WITHOUT its own failing baseline: same skill, same edit,
