...
| **One VM starved under load**    | The lever is `cpuunits` (cgroup weight): raise the priority guest well above the 1024 default (`qm set <vmid> --cpuunits 8000`), leave or lower the batch guests. Weight only biases the scheduler UNDER contention, so batch peak throughput is untouched - unlike `cpulimit`, which hard-caps it. Size vCPUs to the guest's real concurrency (static), NOT via hot-plug, and derive multi-queue counts from the BOOT vCPU count rather than a hot-add ceiling.                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
