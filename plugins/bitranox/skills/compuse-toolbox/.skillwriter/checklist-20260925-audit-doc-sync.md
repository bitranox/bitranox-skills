# skill-writer checklist - compuse-toolbox (2026-09-25, audit behaviour sync)
...
- [x] RED, ground truth: the old rows and paragraphs described behaviour the scripts no longer
      have. Each new claim is pinned by the script's own tests in `tests/`, and the exit-code
      claims were also EXECUTED against the scripts: `procsig --exe ""` and a blank `--cmdline`
      exit 2; `git_state --root <missing>` exits 2; `fleet_ssh --scp a b host:/x` exits 2;
      `backstop --base ""` exits 2; `ci_triage --file ""` and a `--step` that is absent exit 2;
      `mem_levels --root <not an anchor>` exits 2; `conflict_scan <missing path>` exits 2.
- [x] The `statusrot` row named a `--root` flag the script has never had; it now shows
      `scan --chain DIR` / `--level FILE`, which is what `statusrot.py --help` lists.
- [x] Table re-checked with `reformat_tables.py --strict`: canonical, no ragged row.
