# skill-writer checklist - compuse-toolbox (a description that is not valid YAML)
...
`for a recurring chore: finding or killing a process` becomes `chore - finding or killing`.
...
- [x] One breaking colon, at offset 94 of a 999-character description. `yaml.safe_load` over the front-matter block raises
...
- [x] Length re-measured after the edit: 1000 characters, under the 1024 cap the commit gate
      enforces.
