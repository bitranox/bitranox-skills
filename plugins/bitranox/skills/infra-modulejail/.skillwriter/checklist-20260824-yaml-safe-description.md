# skill-writer checklist - infra-modulejail (a description that is not valid YAML)
...
`on an already-jailed host: modprobe exits 0` becomes `host - modprobe exits 0`, and
`journalctl shows "blocked: <module>"` becomes `journalctl logs the module as "blocked"`.
...
- [x] Two breaking colons, at offsets 475 and 583 of a 609-character description. `yaml.safe_load` over the front-matter block raises
...
- [x] Length re-measured after the edit: 613 characters, under the 1024 cap the commit gate
      enforces.
