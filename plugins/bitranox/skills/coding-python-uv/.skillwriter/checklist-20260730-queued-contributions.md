# skill-writer checklist - coding-python-uv: ship queued cross-project contributions (2026-07-30)
...
- [x] Evidence, verbatim where quoted: both explained that `uv run` parses metadata only from the handed file and does not follow imports.
      Shipped on the maintainer's instruction; the bare-environment test-import corollary is the part
      neither scenario probed and is the one that bites CI.
