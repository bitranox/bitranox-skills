...
and the error/exit-code behaviour. ASCII only."""
import os
...
def test_main_exit_code_nonzero_when_out_of_sync(tmp_path, capsys):
    rc = G.main([str(tmp_path)])       # not a git repo -> error path -> rc 1
    assert rc == 1
