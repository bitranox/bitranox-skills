...
| A bare `git` after an earlier `cd`           | The Bash tool's cwd PERSISTS between calls, so a bare `git` answers from whatever repo you last `cd`'d into - a per-repo loop written as `cd "$r" && git ...` silently reports one repo's state for another when a `cd` fails. Pass `-C <repo>` on every invocation instead of `cd`, and sanity-check a sweep's result count against the repo count.                                                                                                                                                                                                                                                                                 |
