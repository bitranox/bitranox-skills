...
| `Cross-Origin-Opener-Policy` | `same-origin-allow-popups` (safe baseline) or `same-origin`                                                                | MINOR                            | isolate browsing context from cross-origin openers (COOP); COEP/CORP are advanced, opt in deliberately |
