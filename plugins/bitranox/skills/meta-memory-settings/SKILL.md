...
| `forgetting`         | `off` [default], `conservative`, `aggressive` | DEFAULT OFF. Forgetting must be USAGE-based (never age, never size); a complete usage meter does not exist yet, so nothing is auto-forgotten. Leave off until usage-based forgetting is built.                                |
| `forget_idle_dreams` | integer [3]                                   | (only relevant if forgetting is ever turned on) age/dream-count threshold - deprecated as a forget metric; do not rely on it.                                                                                                 |
