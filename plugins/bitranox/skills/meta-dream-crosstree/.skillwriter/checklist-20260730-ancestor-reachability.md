# skill-writer checklist - meta-dream-crosstree: judge a CLAUDE.md trim by ancestor reachability (2026-07-30)
...
- [x] Boundaries block reduced to a POINTER at dream-core.md + dream-passes.md.
- [x] Fixed a CIRCULAR cross-reference: this file said the CLAUDE.md policy lives in
      `meta-self-improve`, which said it lives in `meta-dream-tree`; neither landed on the real case
      model. Both now point at dream-passes.md "CLAUDE.md reconciliation".
- [x] The inline "every CLAUDE.md edit stays propose-first and never happens without user
      confirmation" contradicted the new applied-and-reported policy; replaced.
- [x] Report step extended with the per-rule / per-file / covering-ancestor requirement.
