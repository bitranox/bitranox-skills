# skill-writer checklist - meta-dream-crosstree (2026-08-01, close a contribution by outcome)

Change: step 7 carried the same impossible instruction as meta-dream-tree - drain "ONLY for what
actually shipped; leave a contribution queued" - and is corrected the same way, to per-entry `ship
--match` / `drop` with `drain` reserved for an all-shipped sweep. Fixed in the same change as its
sibling, per the fix-a-shared-bug-in-every-sibling rule. Ships with plugin 5.124.0.
