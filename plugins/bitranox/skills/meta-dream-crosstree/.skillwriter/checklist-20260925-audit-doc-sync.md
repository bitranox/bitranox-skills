# skill-writer checklist - meta-dream-crosstree (2026-09-25, audit behaviour sync)
...
- [x] RED, ground truth: the old text said to finish with one `session-reviewed`. `dream_state.py`
      now shows a stretch over 2 MB in parts and `session-reviewed` advances only to the end of
      the part shown (`meta-dream-tree/tests/test_dream_state.py`), so a single call leaves the
      rest unreviewed; the new text loops until `NOTHING NEW`, matching `references/dream-core.md`.
