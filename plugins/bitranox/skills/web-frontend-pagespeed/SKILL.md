...
|-----------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `curl -I` for compression         | Plain `curl -I` sends no `Accept-Encoding`, so nothing is negotiated and `Content-Encoding` is absent whatever the server does. With `--compressed` a server usually DOES report it on a HEAD, but a filter that only runs on a body will not - so a HEAD is unreliable in BOTH directions. Confirm with a GET |
