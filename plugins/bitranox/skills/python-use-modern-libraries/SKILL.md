...
| .gitignore parse / file filtering           | `igittigitt` (git-exact, include mode, memory-bounded; see bitranox:python-gitignore) | hand-rolled `fnmatch`/`glob`/`re`; `gitignore_parser`; `pathspec` |
...
`bitranox:python-enforce-data-architecture-strict` skill.
