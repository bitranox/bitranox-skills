# skill-writer checklist - meta-self-improve (2026-09-25, audit behaviour sync)
...
- [x] RED, ground truth: `references/upstream-propagation.md` said `drain` "clears" the queue.
      `contrib_queue.py drain` now also records each drained entry as shipped, so the intent is
      not re-queued (`tests/test_contrib_queue.py`); the sentence says so.
