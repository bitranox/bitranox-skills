...
|-------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
| "A memory consolidation is due" (session start) | The store has accumulated enough change to be worth a tidy. In a plugin marketplace repo, the memory store, or with `dream_mode` `auto`, it asks you to run `/dream-tree` or say "skip"; in any other project it is one line deferring the dream to a session of its own, so a working session is not pulled into store maintenance | On; silence with the `nudges` knob ([reference.md](reference.md))                                               |
...
| A tooling-detour notice (before a write)        | The call is about to write into the plugin's source repo from a session whose project is something else. Unless you asked for that fix, the symptom is queued (`contrib_queue.py add`) and the work resumes; the dream drains the queue                                                                                             | On; once per session, silent in a dream session and inside the plugin repo itself                               |
