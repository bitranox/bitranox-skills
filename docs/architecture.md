...
| `PreToolUse` Bash  | `sha-literal-nudge.py`                                                                                                                     | Non-blocking: warn on a full sha typed as a literal the session was never shown       |
...
| `PostToolUse` Bash | `ci-watch-nudge.py`                                                                                                                        | After a push that LANDED, name the sha now building and the command to watch it       |
| `Stop`             | `self-improve-gate.py`                                                                                                                     | Detect learning signals in the turn; nudge the capture skill                          |
| `Stop`             | `context-watcher.py`                                                                                                                       | Measure context from transcript usage; offer a handover once past the threshold       |
