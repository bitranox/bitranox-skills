...
| `context_window`       | `0` = detect, or tokens   | `0`            | The context window the handover threshold and the re-ask spacing are measured against. Detected from the model this project has used (`~/.claude.json` keeps the `[1m]` suffix that the transcript drops); `0` means detect. | Set a token count only when detection cannot see your model - the watcher says so explicitly if the window it used is smaller than the context it measured. |
