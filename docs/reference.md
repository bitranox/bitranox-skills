...
| `context_window`       | tokens (integer)          | `200000`       | The real context window this session has. It CANNOT be measured - a transcript records the model as `claude-opus-5`, never the `[1m]` suffix - so it is configured. Wrong value, and the handover offer either never fires or reports itself misconfigured. | Set it to `1000000` on a 1M-context model. If the watcher reports measuring more context than the window, this is the knob it means.                  |
