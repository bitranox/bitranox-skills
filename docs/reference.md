...
| `context_window`       | `0` = detect, or tokens   | `0`            | The context window the handover threshold and the re-ask spacing are measured against. Taken from the model family in the transcript: opus, fable, sonnet and mythos run 1M, haiku 200K. An unknown family falls back to 200K, which asks early rather than going silent. | Set a token count only for a model this table has not met - the watcher says so explicitly if the window it used is smaller than the context it measured. |
