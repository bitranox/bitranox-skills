...
| `classifier_backend`       | `off`, `jev`              | `off`          | Master switch for the opt-in text classifier (TypeSafe's Jev). `off`: no site calls it. `jev`: sites whose own knob is `shadow` send redacted text to `api.typesafe.ai` in a detached process and log Jev's verdict beside the regex's in `~/.claude/self-improve-audit/classifier-shadow.jsonl`; no hook decision changes. Needs an API key (see `TYPESAFE_API_KEY`). | You want to measure whether Jev classifies a site better than its regex, and accept that the named fields (prompt, last assistant message, memory note) leave the machine with secrets replaced by `[REDACTED]`. |
| `classifier_model`         | model id                  | `jev-latest`   | The TypeSafe model id sent with every classifier request.                                                                                                                                                                                                                                                                                                              | Pin a specific version when comparing runs across a model release.                                                                                                                                               |
| `classifier_stop_signal`   | `off`, `shadow`           | `off`          | Shadow-compare the Stop gate's learning-signal regexes: one yes/no question per signal family over the last user message and assistant reply.                                                                                                                                                                                                                          | Together with `classifier_backend = jev`, to collect a comparison for the Stop gate.                                                                                                                             |
| `classifier_skill_router`  | `off`, `shadow`           | `off`          | Shadow-compare the skill router: a new-task question plus one choice over the session's installed skills, beside the keyword match.                                                                                                                                                                                                                                    | Together with `classifier_backend = jev`, to collect a comparison for the router.                                                                                                                                |
| `classifier_recall_rerank` | `off`, `shadow`           | `off`          | Shadow-compare recall ranking: one request per (prompt, memory note) pair over the keyword shortlist of up to 30 notes.                                                                                                                                                                                                                                                | Together with `classifier_backend = jev`, to collect a comparison for recall.                                                                                                                                    |
