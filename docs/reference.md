...
| `BITRANOX_CI_WATCH=1`          | unset   | The post-push CI watch is off: no nudge naming the sha after a push lands, and the Stop gate stops refusing to end a turn whose push was never checked                            | You, at session launch, when you are pushing somewhere whose CI you deliberately are not watching (a docs-only branch, a repo whose runners are down). Leave it unset otherwise: the nudge is what supplies the sha, and the gate is the only thing that makes forgetting loud |
