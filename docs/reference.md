...
| `context_window`       | `0` = detect, or tokens   | `0`            | The context window the handover threshold and the re-ask spacing are measured against. Derived without configuration: the model family from the transcript fixes the ceiling (opus/fable/sonnet 1M-capable, haiku 200K) and the largest context observed proves which variant is running. | Set a token count only if the derivation is wrong for your setup - the watcher says so explicitly if the window it used is smaller than the context it measured. |
